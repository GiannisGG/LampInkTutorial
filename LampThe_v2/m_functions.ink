...
If it is lower that the newState, it assigns
the newState to the stateMachine.
Else, it does nothing.

Note that the stateMachine argument is
passed by reference, so the function
can actually alter it
...
No biggie, don't let it scare you.
As you see, it ends with a divert to ->beIn.
A function wouldn't be allowed to do that.
*/

//  Before changing rooms, we first assign
//  the current room to the variable previousRoom:
    ~ previousRoom = currentRoom
//  ... and the same with its knot address:
    ~ previousRoomAddress = currentRoomAddress
//  Remember: we declared the var previousRoomAddress
//  as one that holds the value of a divert.
//  Divert: the ->thingy.
    
//  Finally, we call another knot, passing the same
//  argument:
    ->beIn(targetRoom)
...
*/
    -> DONE
/*
This shows Inky we acknowledgge that
all the diverts we need will be
provided by the above threads.
*/
...
*/
    = checkItems
    /*
    Oh, this is a "stitch," by the way.
    A stitch is... a sub-knot.
    */
...
there is a hall there, so you get the option to 
"Return to the hall," instead.
